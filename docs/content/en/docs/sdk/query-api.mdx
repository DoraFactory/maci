...
// state while the time window is open.
```
...
console.log('Poll ID:', pollId);  // e.g., 42
```
...
console.log('balance:', status.balance);           // Voice credits if registered
```
...
console.log('Vote counts per option:', results);  // Vec<Uint256>
```
