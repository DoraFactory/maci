...
async function platformPrepareRound() {
  ...
  const round = await client.createAMaciRound({
    ...
    // PrePopulated registration mode (replaces preDeactivateRoot + preDeactivateCoordinator)
    registrationMode: {
      pre_populated: {
        pre_deactivate_root: preDeactivateTree.root,
...
4. **Create Round**
   ...
   - Configure `registrationMode` as `pre_populated` with `pre_deactivate_root` and `pre_deactivate_coordinator`
   - Configure `voiceCreditMode` (e.g., `unified` with fixed amount)
...
- Pre-deactivate tree  
- User's new keypairs (user-generated locally) - NO
- Which address user uses to send transaction - NO
- User's vote content - NO

### What Operator Knows

- Someone registered with pre-add-new-key  
- They are one in pre-deactivate tree  
- Which specific user number - NO
- Which address sent transaction - NO
- User's real identity - NO
