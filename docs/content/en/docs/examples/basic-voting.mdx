  ...
  const round = await client.createAMaciRound({
    ...
    registrationMode: {
      sign_up_with_static_whitelist: {
        whitelist: {
          users: [
            { addr: process.env.VOTER_A_ADDRESS! },
            { addr: process.env.VOTER_B_ADDRESS! },
            { addr: process.env.VOTER_C_ADDRESS! }
          ]
        }
      }
    },
...
- [Pre-add-new-key Example](/docs/examples/pre-addnewkey-round) - Anonymous voting example
- [SDK Documentation](/docs/sdk/installation) - Learn more SDK features
- [Protocol Explained](/docs/protocol/overview) - Deep dive into MACI
