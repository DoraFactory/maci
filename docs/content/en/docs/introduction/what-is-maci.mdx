...
AMACI uses a `RegistrationModeConfig` enum to control how users register. Three modes are available:

**`SignUpWithStaticWhitelist`** is the fastest. Pre-defined addresses call SignUp directly, suitable for public voting scenarios. The downside is the operator can see your address.

**`SignUpWithOracle`** enables oracle-gated registration. Any address can register but must provide a backend-signed certificate. Voice credits can be fixed (Unified) or variable per user (Dynamic).

**`PrePopulated`** (Pre-add-new-key) combines anonymity and speed. Vote organizers pre-configure an anonymity set, and users can register anonymously immediately via ZK proof — without linking to their real identity.

When `deactivateEnabled: true`, the **Add-new-key** flow is also available, allowing users to create an anonymous identity from within the round itself after deactivating their original key.

All vote messages now include a **Poll ID** to prevent cross-round replay attacks.
...
- Smart Contracts (Rust/CosmWasm) - Deployed on [Dora Vota chain](http://vota-explorer.dorafactory.org/)
