...
async function createVote() {
  ...
  const round = await client.createAMaciRound({
    ...
    // Voice credit allocation: every voter gets 100 credits
    voiceCreditMode: { unified: { amount: '100' } },

    // Registration: only these addresses can sign up
    registrationMode: {
...
async function castVote() {
  ...
  // Get round info and operator public key
...
Your address is not in the `SignUpWithStaticWhitelist`. Check:
- Did you include your address in `registrationMode.sign_up_with_static_whitelist.whitelist.users` when creating the round?
- Are you signing with the same address that was whitelisted?
