...
const roundParams = {
  ...
  // Voice credit mode: all users get 100 credits
  voiceCreditMode: { unified: { amount: '100' } },
  
  // Registration mode: static whitelist
  registrationMode: {
...
console.log('Note: User A\'s address must be in whitelist');
...
console.log('Note: Operator can know User A\'s identity through on-chain transactions');
```
...
async function completeVotingRound() {
  ...
  const round = await client.createAMaciRound({
    ...
    circuitType: MaciCircuitType.QV,
    whitelist: {
      addresses: [addressA, addressB]  // Whitelist addresses
    },
  });
