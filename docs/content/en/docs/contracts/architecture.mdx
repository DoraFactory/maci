...
ExecuteMsg::CreateRound {
    ...
    deactivate_enabled,          // New: enable/disable deactivate feature
    voice_credit_mode,           // New: Unified { amount } or Dynamic
    registration_mode,           // New: combined access control enum
}
...
> **Note:** The `Voting` state was removed. The contract determines whether the voting window is open by comparing block time against `start_time` and `end_time`. Users signup and vote while the contract is in `Pending` state and within the time window.
...
```rust
// Key storage items in Registry contract
ADMIN: Item<Admin>                                // Admin address
OPERATOR: Item<Addr>                              // Registry operator address
AMACI_CODE_ID: Item<u64>                          // AMACI code ID for contract instantiation

// Operator network
MACI_VALIDATOR_LIST: Item<ValidatorSet>
MACI_VALIDATOR_OPERATOR_SET: Map<&Addr, Addr>
MACI_OPERATOR_PUBKEY: Map<&Addr, PubKey>
MACI_OPERATOR_IDENTITY: Map<&Addr, String>
FEEGRANT_OPERATOR: Map<&Addr, bool>

// Fee configuration
CIRCUIT_CHARGE_CONFIG: Item<CircuitChargeConfig>

// Poll ID management (new)
NEXT_POLL_ID: Item<u64>               // Auto-incrementing counter
POLL_ID_TO_ADDRESS: Map<u64, Addr>    // poll_id → contract address
ADDRESS_TO_POLL_ID: Map<&Addr, u64>   // contract address → poll_id
```

### AMACI State

```rust
// Key storage items in AMACI contract
POLL_ID: Item<u64>                    // Unique poll ID assigned by Registry
PERIOD: Item<Period>                  // Current round period status (Pending/Processing/Tallying/Ended)
MACIPARAMETERS: Item<MaciParameters>  // ZK circuit parameters

// Unified configuration (new)
VOICE_CREDIT_MODE: Item<VoiceCreditMode>      // Unified { amount } or Dynamic
REGISTRATION_MODE: Item<RegistrationMode>     // SignUpWithStaticWhitelist / SignUpWithOracle / PrePopulated
DEACTIVATE_ENABLED: Item<bool>               // Whether deactivate→AddNewKey flow is active

// User data
NUMSIGNUPS: Item<Uint256>
WHITELIST: Item<Whitelist>           // Used in SignUpWithStaticWhitelist mode
ORACLE_WHITELIST: Map<...>           // Used in SignUpWithOracle mode

// Message storage
MSG_CHAIN_LENGTH: Item<Uint256>      // Total vote message count
DMSG_CHAIN_LENGTH: Item<Uint256>     // Total deactivate message count

// Merkle tree state
CURRENT_STATE_COMMITMENT: Item<Uint256>
CURRENT_TALLY_COMMITMENT: Item<Uint256>
CURRENT_DEACTIVATE_COMMITMENT: Item<Uint256>

// Round configuration
ROUNDINFO: Item<RoundInfo>
VOTINGTIME: Item<VotingTime>
COORDINATORHASH: Item<Uint256>

// Voting configuration
MAX_VOTE_OPTIONS: Item<Uint256>
VOTEOPTIONMAP: Item<Vec<String>>
CIRCUITTYPE: Item<Uint256>
CERTSYSTEM: Item<Uint256>

// Results
RESULT: Map<Vec<u8>, Uint256>
TOTAL_RESULT: Item<Uint256>
```
...
ExecuteMsg::UpdateAmaciCodeId {
    code_id: u64,  // Note: parameter renamed from amaci_code_id to code_id
}
