...
sequenceDiagram
    ...
    User->>SDK: createAMaciRound(params)
...
// Method 1: Static whitelist mode (SignUpWithStaticWhitelist + Unified VC)
const roundWithWhitelist = await client.createAMaciRound({
  signer: wallet,
  ...
  // 6. Voice credit mode: all users get 100 credits
  voiceCreditMode: { unified: { amount: "100" } },
  
  // 7. Registration mode: static whitelist
  registrationMode: {
  ...
  // 8. Deactivate feature (optional, default false)
  deactivateEnabled: false
});

// Method 2: PrePopulated mode (anonymous registration via pre-configured keys)
const roundWithPreKeys = await client.createAMaciRound({
  signer: wallet,
  operator: selectedOperator.address,
  startVoting: new Date('2024-03-01T00:00:00Z'),
  endVoting: new Date('2024-03-07T23:59:59Z'),
  title: 'Anonymous Community Vote',
  voteOptionMap: ['Option A', 'Option B', 'Option C'],
  maxVoter: 100,
  circuitType: MaciCircuitType.QV,
  voiceCreditMode: { unified: { amount: "100" } },
  
  // PrePopulated: users must register via PreAddNewKey
  registrationMode: {
    pre_populated: {
      pre_deactivate_root: preGeneratedRoot,
...
console.log('Poll ID:', await client.getPollId({ contractAddress: round.contractAddress }));
...
const round = await client.createAMaciRound({
  ...
  registrationMode: {
    sign_up_with_static_whitelist: {
      whitelist: { users: [{ addr: 'dora1...' }, { addr: 'dora1...' }] }
    }
  },
  deactivateEnabled: false,
  // ... other params
...
const round = await client.createAMaciRound({
  ...
  operator: selectedOperator.address,
  // ... other round parameters
  voiceCreditMode: { unified: { amount: '100' } },
  registrationMode: {
    sign_up_with_static_whitelist: {
      whitelist: { users: whitelistAddresses.map(addr => ({ addr })) }
    }
  },
  deactivateEnabled: false
});
...
The Registry assigns a unique, sequential **Poll ID** to every AMACI contract it creates. This ID serves as a cross-round replay attack prevention mechanism — each vote message is cryptographically bound to its `poll_id`, so a message signed for round 5 cannot be replayed in round 6.

### Poll ID Queries

```typescript
// Get the Poll ID of an AMACI contract by its address
const pollId = await client.getPollId({ contractAddress: amaciAddress });
// or query the Registry directly:
// QueryMsg::GetPollId { address: Addr }  -> u64

// Reverse lookup: get contract address from Poll ID
// QueryMsg::GetPollAddress { poll_id: u64 } -> Option<Addr>

// Get the next Poll ID that will be assigned
// QueryMsg::GetNextPollId {} -> u64

// Get the current AMACI Code ID used for instantiation
// QueryMsg::GetAmaciCodeId {} -> u64
```

Poll IDs start at `1` and increment by one for each new round created through Registry.
