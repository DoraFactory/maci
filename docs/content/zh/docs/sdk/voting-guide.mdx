...
```typescript
// 统一查询接口，替代原来的 isWhitelisted / queryWhitelistBalanceOf
const status = await client.queryRegistrationStatus({
  contractAddress: amaciContractAddress,
  address: userAddress  // SignUpWithStaticWhitelist 模式下使用
});

console.log('可以注册:', status.can_sign_up);
console.log('是否已注册:', status.is_register);
console.log('投票权重:', status.balance);
```
