...
async function castVote() {
  ...
  await client.vote({
    ...
    contractAddress,
    selectedOptions: [
      { idx: 0, vc: 8 },  // 提案 A：8 票（QV 模式消耗 64 积分）
      { idx: 2, vc: 6 }   // 提案 C：6 票（QV 模式消耗 36 积分）
    ],
    operatorCoordPubKey: [
...
你的地址不在 `SignUpWithStaticWhitelist` 中。检查：
- 创建轮次时是否将该地址加入 `registrationMode.sign_up_with_static_whitelist.whitelist.users`
- 签名使用的地址是否与白名单中的地址一致
