...
| 参数 | 值 | 依据 |
|------|---|------|
| `PER_VOTE_DELAY` | **2 秒/票** | 参考 4-2-2-25（0.2531s）和 6-3-3-125（0.2289s），在 1s 基础上额外增加安全裕量 |
...
tally_timeout = 240 + 172800 = 173040s ≈ 2 天 4 分
```
...
tally_timeout = 3540 + 172800 = 176340s ≈ 2 天 59 分
```

### 6-3-3-125，10000 票

```
delay_allowed = (1380 + 10000 × 2) × 3 = 64140s ≈ 17.8 小时
tally_timeout = 64140 + 172800 = 236940s ≈ 2.74 天
```
...
```mermaid
flowchart TD
    A["execute_claim 调用"] --> B{"当前时间 > voting_end + tally_timeout?"}
    B -->|"是，已超时"| C["100% → admin\nfee_recipient 获得 0%"]
    B -->|"否"| D{"period == Ended?"}
    D -->|"否"| E["返回错误 PeriodError"]
    D -->|"是"| F["10% → fee_recipient\n剩余 90% 待分配"]
    F --> G["计算 penalty_rate"]
    G --> H["TallyDelay：每条记录 +50%"]
    G --> I["DeactivateDelay：每条超时消息 +5%\n上限 100%"]
    H --> J["miss_rate = 100% - penalty_rate"]
    I --> J
    J --> K["operator_reward = 90% × miss_rate → operator"]
    J --> L["penalty_amount = 90% × penalty_rate → admin"]
```

> **注意**：`fee_recipient`（由 registry 在创建轮次时指定）与 `admin`（round 的 admin）是不同地址。超时时 `fee_recipient` 拿不到任何资金。

| 情形 | fee_recipient 获得 | operator 获得 | admin 获得 |
|------|-------------------|-------------|----------|
| 无延迟，正常完成 | 10% | 90% | 0% |
| 1 次 TallyDelay | 10% | 45% | 45% |
| 2 次 TallyDelay | 10% | 0% | 90% |
| N 条 DeactivateDelay 消息 | 10% | 90% − N×4.5% | N×4.5% |
| 超过 tally_timeout | **0%** | 0% | 100% |
...
| 统一部分 | Vote Fee = 0.06 DORA/票（统一） | Per-Vote Delay = 2s/票（统一） |
