...
// 方式 1: 静态白名单模式（SignUpWithStaticWhitelist + Unified VC）
...
// 方式 2: PrePopulated 模式（通过预配置密钥进行匿名注册）
const roundWithPreKeys = await client.createAMaciRound({
  ...
  // PrePopulated：用户必须通过 PreAddNewKey 注册
  registrationMode: {
    pre_populated: {
...
const round = await client.createAMaciRound({
  ...
  // ... 其他参数
  voiceCreditMode: { unified: { amount: "100" } },
  registrationMode: { sign_up_with_static_whitelist: { whitelist: { users: [] } } },
  deactivateEnabled: false
});
...
Registry 为其创建的每个 AMACI 合约分配唯一的、顺序递增的 **Poll ID**。该 ID 是防止跨轮次重放攻击的机制——每条投票消息都在密码学上绑定到其 `poll_id`，因此为第 5 轮签名的消息无法在第 6 轮中重放。
