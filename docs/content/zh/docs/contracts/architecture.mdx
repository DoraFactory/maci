...
> **注意：** `Voting` 状态已移除。合约通过比较当前区块时间与 `start_time` 和 `end_time` 来判断投票窗口是否开放。用户在合约处于 `Pending` 状态且时间窗口内完成签到和投票。
...
```rust
// Registry 合约关键存储项
ADMIN: Item<Admin>                                // 管理员地址
OPERATOR: Item<Addr>                              // Registry Operator 地址
AMACI_CODE_ID: Item<u64>                          // 用于合约实例化的 AMACI Code ID
...
// Poll ID 管理（新增）
NEXT_POLL_ID: Item<u64>               // 自动递增计数器
POLL_ID_TO_ADDRESS: Map<u64, Addr>    // poll_id → 合约地址
ADDRESS_TO_POLL_ID: Map<&Addr, u64>   // 合约地址 → poll_id
```
...
```rust
// AMACI 合约关键存储项
POLL_ID: Item<u64>                    // Registry 分配的唯一 Poll ID
PERIOD: Item<Period>                  // 当前轮次状态（Pending/Processing/Tallying/Ended）
MACIPARAMETERS: Item<MaciParameters>  // ZK 电路参数

// 统一配置（新增）
VOICE_CREDIT_MODE: Item<VoiceCreditMode>      // Unified { amount } 或 Dynamic
REGISTRATION_MODE: Item<RegistrationMode>     // SignUpWithStaticWhitelist / SignUpWithOracle / PrePopulated
DEACTIVATE_ENABLED: Item<bool>               // 是否开启 deactivate→AddNewKey 流程

// 用户数据
NUMSIGNUPS: Item<Uint256>
WHITELIST: Item<Whitelist>           // SignUpWithStaticWhitelist 模式使用
ORACLE_WHITELIST: Map<...>           // SignUpWithOracle 模式使用

// 消息存储
MSG_CHAIN_LENGTH: Item<Uint256>      // 投票消息总数
DMSG_CHAIN_LENGTH: Item<Uint256>     // Deactivate 消息总数
