...
1. **活性保证**：确保 Operator 会及时处理，不会拒绝服务
2. **质量保证**：专业团队运维，减少错误
3. **可靠性**：经过验证的节点，有历史记录可查
4. **责任制**：有奖罚机制约束行为
...
sequenceDiagram
    ...
    User->>SDK: createOracleMaciRound(参数)
...
现在您已经了解了 Registry 和 Operator 网络，接下来可以：

- 🗳️ [AMACI 合约](/docs/contracts/amaci) - 了解投票合约和三种注册方式
