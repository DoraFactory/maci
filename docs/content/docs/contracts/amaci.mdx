...
sequenceDiagram
    ...
    User->>Oracle: 1. 请求证书
    Oracle->>Oracle: 验证白名单资格
    Oracle-->>User: 返回 (amount, signature)
    User->>AMACI: 2. Signup(pubkey, amount, signature)
    AMACI->>AMACI: 3. 验证签名
    alt 签名无效
        AMACI-->>User: 错误：签名验证失败
    end
    AMACI->>AMACI: 4. 检查投票期
    alt 不在投票期
        AMACI-->>User: 错误：不在投票期
    end
    AMACI->>Storage: 5. 添加用户到状态树
    AMACI->>AMACI: 6. 分配状态索引
...
1. **固定权重模式**：所有白名单用户获得相同投票权重
2. **动态权重模式**：基于代币持有量等链上数据计算权重
...
await client.createMaciRound({
  ...
  voiceCreditsPerUser: 100  // 固定权重模式
});
```
...
现在您已经了解了 AMACI 合约的所有注册方式，接下来可以：
