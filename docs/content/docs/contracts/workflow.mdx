...
console.log('⚠️  注意：用户 A 的地址必须在白名单中');
...
console.log('⚠️  注意：Operator 可以通过链上交易知道用户 A 的身份');
```
...
// 步骤 1: 等待 Operator 处理 deactivate
console.log('等待 Operator 处理 deactivate 消息...');
// （这个由 Operator 自动完成）

// 步骤 2: 获取 deactivate 数据
...
if (deactivateMessages.length > 0) {
  ...
  // 步骤 2: 处理并生成 deactivate tree
  // （Operator 使用专业工具自动完成）
  
  // 步骤 3: 提交 ProcessDeactivate 证明
