...
// Adapter class to make SDK Tree compatible with test expectations
class IncrementalQuinTree {
  private tree: Tree;
  private nextIndex: number = 0;

  constructor(depth: number, zero: bigint, degree: number, _hashFunc?: any) {
    this.tree = new Tree(degree, depth, zero);
  }
  ...
  insert(leaf: bigint): void {
    this.tree.updateLeaf(this.nextIndex, leaf);
    this.nextIndex++;
  }

  generateProof(index: number): { pathElements: bigint[][]; pathIndices: bigint[] } {
    const pathElements = this.tree.pathElementOf(index);
    const pathIndices = this.tree.pathIdxOf(index);
...
describe('Incremental Quinary Tree (IQT)', function test() {
    ...
    it('should throw error if input is out of bounds [fuzz]', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.bigInt({ min: 1n, max: SNARK_FIELD_SIZE - 1n }),
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (levels: bigint, input: bigint) => {
            ...
            const isEqual = pathIndices.every(
              (item, index) => item.toString() === values[index].toString()
            );
  ...
  describe('QuinaryLeafExists', () => {
    ...
    it('should throw when provided an incorrect leaf', async () => {
      ...
      await expect(circuitLeafExists.calculateWitness(circuitInputs)).to.be.rejectedWith(
        'Assert Failed.'
      );
    });
