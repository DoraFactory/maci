...
async function main() {
  ...
  const operator = 'dora1zgkjgh2ylxnq7x3v5cws4e2tzszn34yy7tee6w';
  ...
  const deactivateData = await voterClient.saasGetPreDeactivate(contractAddress);

  const preDeactivateCoordPubkey = BigInt(deactivateData.coordinator);
