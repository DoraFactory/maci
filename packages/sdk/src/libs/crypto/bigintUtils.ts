...
export const unstringifyBigInts = (
  ...
  if (typeof input === 'object') {
    return Object.entries(input).reduce<Record<string, bigint>>(
      (acc, [key, value]) => {
 ...
 */
export const fromRprLE = (
  buff: ArrayBufferView,
  o = 0,
  n8: number = buff.byteLength
): string => {
...
export const stringifyBigInts = (input: BigIntVariants): StringifiedBigInts => {
  ...
  if (typeof input === 'object') {
    return Object.entries(input).reduce<Record<string, StringifiedBigInts>>(
      (acc, [key, value]) => {
